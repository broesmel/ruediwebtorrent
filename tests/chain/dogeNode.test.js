...
describe('generateAddress', () => {
  it('calls getnewaddress with a user label and returns { address, userId }', async () => {
    mockFetch.mockResolvedValueOnce(rpcOk('DDogeDummyAddress123'))

    const result = await generateAddress(7)

    expect(result).toEqual({ address: 'DDogeDummyAddress123', userId: 7 })
    const body = JSON.parse(mockFetch.mock.calls[0][1].body)
    expect(body.method).toBe('getnewaddress')
    expect(body.params[0]).toContain('7')
  })
...
describe('getConfirmedDeposits', () => {
  ...
  it('fetches block hash when sinceHeight > 0', async () => {
    mockFetch
      .mockResolvedValueOnce(rpcOk('dogeblockhash456'))           // getblockhash
      .mockResolvedValueOnce(rpcOk({ transactions: [] }))          // listsinceblock

    await getConfirmedDeposits(200)

    const hashCall = JSON.parse(mockFetch.mock.calls[0][1].body)
    expect(hashCall.method).toBe('getblockhash')
    expect(hashCall.params[0]).toBe(200)

    const listCall = JSON.parse(mockFetch.mock.calls[1][1].body)
    expect(listCall.method).toBe('listsinceblock')
    expect(listCall.params[0]).toBe('dogeblockhash456')
  })

  it('maps receive transactions to deposit objects', async () => {
    const txs = [
      { category: 'receive', address: 'DAddr1', amount: 1000, txid: 'dogetx1', confirmations: 10 },
      { category: 'send',    address: 'DAddr2', amount: 500,  txid: 'dogetx2', confirmations: 10 },  // filtered out
    ]
    ...
    expect(deposits[0]).toEqual({ address: 'DAddr1', amount: 1000, txid: 'dogetx1', confirmations: 10 })
  })

  it('filters out under-confirmed transactions', async () => {
    const txs = [
      { category: 'receive', address: 'D1', amount: 500, txid: 'dogetx1', confirmations: 3 },
    ]
    mockFetch.mockResolvedValueOnce(rpcOk({ transactions: txs }))
...
describe('getCurrentHeight', () => {
  it('returns current block count', async () => {
    mockFetch.mockResolvedValueOnce(rpcOk(5_000_000))
    expect(await getCurrentHeight()).toBe(5_000_000)
  })
})
