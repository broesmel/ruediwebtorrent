...
describe('generateAddress', () => {
  it('calls getnewaddress with a user label and returns { address, userId }', async () => {
    mockFetch.mockResolvedValueOnce(rpcOk('1BtcDummyAddress123'))

    const result = await generateAddress(42)

    expect(result).toEqual({ address: '1BtcDummyAddress123', userId: 42 })
...
describe('getConfirmedDeposits', () => {
  ...
  it('fetches block hash when sinceHeight > 0', async () => {
    mockFetch
      .mockResolvedValueOnce(rpcOk('btcblockhash123'))           // getblockhash
    ...
    expect(listCall.params[0]).toBe('btcblockhash123')
  })

  it('maps receive transactions to deposit objects', async () => {
    const txs = [
      { category: 'receive', address: '1BtcAddr1', amount: 0.001, txid: 'btctx1', confirmations: 10 },
      { category: 'send',    address: '1BtcAddr2', amount: 0.5,   txid: 'btctx2', confirmations: 10 },  // filtered out
    ]
    ...
    expect(deposits[0]).toEqual({ address: '1BtcAddr1', amount: 0.001, txid: 'btctx1', confirmations: 10 })
  })

  it('filters out under-confirmed transactions', async () => {
    const txs = [
      { category: 'receive', address: '1B1', amount: 1, txid: 'btctx1', confirmations: 2 },
    ]
    mockFetch.mockResolvedValueOnce(rpcOk({ transactions: txs }))
...
describe('getCurrentHeight', () => {
  it('returns current block count', async () => {
    mockFetch.mockResolvedValueOnce(rpcOk(850_000))
    expect(await getCurrentHeight()).toBe(850_000)
  })
})
