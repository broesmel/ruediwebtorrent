...
const HOST          = process.env.DOGE_RPC_HOST  ?? '127.0.0.1'
const PORT          = process.env.DOGE_RPC_PORT  ?? '22555'
const USER          = process.env.DOGE_RPC_USER  ?? ''
const PASS          = process.env.DOGE_RPC_PASS  ?? ''
const CONFIRMATIONS = Number(process.env.DOGE_CONFIRMATIONS ?? 6)
...
async function rpc(method, params = []) {
  ...
  if (!res.ok) throw new Error(`DOGE RPC HTTP ${res.status} on ${method}`)
  const json = await res.json()
  if (json.error) throw new Error(`DOGE RPC: ${json.error.message}`)
  return json.result
}

/**
 * Generate a new Dogecoin address for a user.
