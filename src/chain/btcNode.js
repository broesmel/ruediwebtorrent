...
const PASS          = process.env.BTC_RPC_PASS  ?? ''
const CONFIRMATIONS = Number(process.env.BTC_CONFIRMATIONS ?? 6)
...
async function rpc(method, params = []) {
  ...
  if (!res.ok) throw new Error(`BTC RPC HTTP ${res.status} on ${method}`)
